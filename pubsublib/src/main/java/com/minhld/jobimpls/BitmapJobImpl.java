 ...
 */
public class BitmapJobImpl implements Job {
    long startJob = 0;
    int prevPerc = 0;
    String outputFile = Utils.getDownloadPath() + "/bmp_" + System.currentTimeMillis() + ".txt";
    PrintWriter writer = null;

    public Object exec(Object dataObject) {
        ...
        Bitmap orgBmp = (Bitmap) dataObject;
        ...
        for (x = 0; x < w; x++) {
            ...
            // calculate percentage
            perc = 0.5 + 0.5 * ((double) x / (double) w);
            writeTime(writer, (int) Math.ceil(perc * 100));
        }
